...
export type HanimeEpisodeHistory = {
  videoCode: string
  sourceKey: string
  sourceLabel: string
  sourceUrl: string
...
class HanimeDatabase {
  ...
  async addHistory(video: HanimeVideoItem, episode?: { sourceKey: string; sourceLabel: string; sourceUrl: string }): Promise<void> {
    ...
    if (episode) {
      await this.db!.execute(
        `INSERT INTO hanime_episode_history (video_code, source_key, source_label, source_url, play_count, last_watched_at)
         VALUES (?, ?, ?, ?, 1, ?)
         ON CONFLICT(video_code, source_key) DO UPDATE SET
           source_label = excluded.source_label,
           source_url = excluded.source_url,
           play_count = hanime_episode_history.play_count + 1,
           last_watched_at = excluded.last_watched_at`,
        [video.videoCode, episode.sourceKey, episode.sourceLabel, episode.sourceUrl, now]
      )
    }
  }

  async getEpisodeHistory(videoCode: string): Promise<HanimeEpisodeHistory[]> {
    await this.ensureReady()
    const rows = await this.db!.fetchAll<any>(
      "SELECT video_code, source_key, source_label, source_url, play_count, last_watched_at FROM hanime_episode_history WHERE video_code = ? ORDER BY last_watched_at DESC",
      [videoCode]
    )
    return rows.map((row) => ({
      videoCode: String(row.video_code),
      sourceKey: String(row.source_key),
      sourceLabel: String(row.source_label || "剧集"),
  ...
  async clearFavorites(): Promise<void> {
    await this.ensureReady()
    await this.db!.execute(
      "UPDATE hanime_video SET is_favorite = 0, updated_at = ? WHERE is_favorite = 1",
      [Date.now()]
    )
  }
  ...
  async clearHistory(): Promise<void> {
    ...
    await this.db!.execute("DELETE FROM hanime_episode_history")
  }
  ...
  private async createTables(): Promise<void> {
    ...
    `)
    await this.db!.execute(`
      CREATE TABLE IF NOT EXISTS hanime_episode_history (
        ...
        PRIMARY KEY(video_code, source_key),
    ...
    await this.db!.execute("CREATE INDEX IF NOT EXISTS idx_hanime_episode_history_time ON hanime_episode_history(video_code, last_watched_at DESC)")
