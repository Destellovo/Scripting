...
export type HanimeHomePage = {
  banners: HanimeHomeBanner[]
  banner?: HanimeHomeBanner
  sections: HanimeHomeSection[]
  timestamp: number
}

export type HanimeWeekdayId = 1 | 2 | 3 | 4 | 5 | 6 | 7

export type HanimeWeekdayScheduleDay = {
  id: HanimeWeekdayId
  title: string
  shortTitle: string
  items: HanimeVideoItem[]
...
export type HanimeWeekdaySchedule = {
  days: HanimeWeekdayScheduleDay[]
  timestamp: number
}
...
class HanimeClient {
  private homeCache: HanimeHomePage | null = null
  private homeCacheAt = 0
  private weekdayScheduleCache: HanimeWeekdaySchedule | null = null
  private weekdayScheduleCacheAt = 0

  async getHomePage(forceRefresh = false): Promise<HanimeHomePage> {
    const cachedPage = this.homeCache
    const cacheIsFresh = cachedPage !== null && Date.now() - this.homeCacheAt < 5 * 60 * 1000
    if (!forceRefresh && cacheIsFresh) return cachedPage

    const html = await this.fetchHtml("")
    const page = parseHomePage(html)
    this.homeCache = page
    this.homeCacheAt = Date.now()
    return page
  }

  async getWeekdaySchedule(forceRefresh = false): Promise<HanimeWeekdaySchedule> {
    const cachedSchedule = this.weekdayScheduleCache
    const cacheIsFresh = cachedSchedule !== null && Date.now() - this.weekdayScheduleCacheAt < 5 * 60 * 1000
    if (!forceRefresh && cacheIsFresh) return cachedSchedule

    const html = await this.fetchHtml("")
    const schedule = parseWeekdaySchedule(html)
    this.weekdayScheduleCache = schedule
    this.weekdayScheduleCacheAt = Date.now()
    return schedule
  }
  ...
  private async searchSuggest(query: string): Promise<HanimeVideoItem[]> {
    ...
    const response = await this.request(url, "application/json,text/plain,*/*;q=0.8")
  ...
  private async fetchHtml(path: string): Promise<string> {
    const response = await this.request(buildUrl(path), "text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8")
  ...
  private async request(url: string, accept: string) {
    ...
    for (let attempt = 0; attempt < 2; attempt += 1) {
      ...
      try {
        ...
        if (!response.ok) {
          if (response.status === 403 || response.status === 429) throwHttpError(response.status)
          if (response.status < 500 || attempt === 1) throwHttpError(response.status)
        ...
        lastError = error
        if (attempt === 1 || isAccessRestrictedError(error)) break
      } finally {
        clearTimeout(timer)
      }
      await delay(500 * (attempt + 1))
    }
    if (isAbortError(lastError)) throw new Error("请求超时，请检查网络后重试。")
    throw lastError instanceof Error ? lastError : new Error(`${lastError || "请求失败"}`)
  }
}
...
function parseWeekdaySchedule(html: string): HanimeWeekdaySchedule {
  const weekdayTitles: Array<{ id: HanimeWeekdayId; title: string; shortTitle: string }> = [
  ...
  const days = weekdayTitles.map((weekday) => {
    const segment = extractElementById(html, `week-module-${weekday.id}`) || ""
    return {
      ...weekday,
      items: segment ? parseVideoItems(segment) : [],
    }
  })

  if (!days.some((day) => day.items.length > 0)) {
    throw new Error("官网时间表暂未返回番剧条目，请稍后刷新重试。")
  }
...
function parseHomePage(html: string): HanimeHomePage {
  ...
  const ratingsByVideoCode = new Map(
    parseVideoItems(html)
      .filter((item) => item.ageRating)
      .map((item) => [item.videoCode, item.ageRating] as const),
  )
  const banners = parseBanners(html).map((banner) => ({
    ...banner,
    ageRating: banner.videoCode ? ratingsByVideoCode.get(banner.videoCode) : undefined,
  }))
  ...
  return { banners, banner: banners[0], sections, timestamp: Date.now() }
}

function parseBanners(html: string): HanimeHomeBanner[] {
  const slides = extractElementsByClass(html, "slide-time-bj")
  const fallbackSlides = slides.length > 0 ? slides : extractElementsByClass(html, "slide-a")
  const banners: HanimeHomeBanner[] = []
  ...
  for (const slide of fallbackSlides) {
    ...
    const identity = videoCode || `${title}|${image?.url || ""}`
...
function parseVideoItems(html: string): HanimeVideoItem[] {
  ...
  while ((match = regex.exec(html)) != null) {
    ...
    result.push({
      ...
      ageRating: extractAgeRating(segment),
...
function extractAgeRating(segment: string): string | undefined {
  const label = extractTextByClass(segment, "public-prt") || ""
  const rating = /(?:普通級|普通级|PG-12|R-15|R-18)/i.exec(label)?.[0]
  return rating ? rating.replace("普通级", "普通級").toUpperCase() : undefined
}
...
function extractElementsByClass(html: string, classPart: string): string[] {
  const result: string[] = []
  const regex = new RegExp(`<([a-zA-Z][\\w:-]*)\\b[^>]*class=["'][^"']*${escapeRegExp(classPart)}[^"']*["'][^>]*>`, "gi")
  let match: RegExpExecArray | null
  while ((match = regex.exec(html)) != null) {
    const element = extractElementAt(html, match.index)
